...
CHARACTER_CONFIG = {
    ...
    "options": {
        ...
        "color": {
            "values": [
                {"name": "Light Blue", "value": (52, 222, 235)},
        ...
        "origin": {
            "values": [
                {"name": "Explorer", "equipment_points": 10, "wealth": 0},
                {"name": "Traveller", "equipment_points": 8, "wealth": 10},
                {"name": "Scholar", "equipment_points": 6, "wealth": 20},
                {"name": "Nomad", "equipment_points": 4, "wealth": 30},
                {"name": "Local", "equipment_points": 2, "wealth": 40}
            ],
            "display_name": "Origin"
        },
        "background": {
            "values": [
                {"name": "Royalty", "skill_points": 0, "wealth": 50},
                {"name": "Wealthy", "skill_points": 2, "wealth": 40},
                {"name": "Comfortable", "skill_points": 4, "wealth": 30},
                {"name": "Meager", "skill_points": 6, "wealth": 20},
                {"name": "Poor", "skill_points": 8, "wealth": 10},
                {"name": "Squalid", "skill_points": 10, "wealth": 0}
            ],
            "display_name": "Background"
        }
    },
    "menu_layout": ["Name", "Symbol", "Color", "Origin", "Background", "Start Game", "Back"]
}
...
class CharacterCustomizationScreen(input_handlers.BaseEventHandler):
    ...
    def __init__(self, parent_menu):
        ...
        self.character_data["Skill Points"] = CHARACTER_CONFIG["defaults"]["skill_points"]
        self.character_data["Equipment Points"] = CHARACTER_CONFIG["defaults"]["equipment_points"]
        self.character_data["Wealth"] = CHARACTER_CONFIG["defaults"]["wealth"]
        ...
        # Calculate initial derived stats based on default selections
        self.recalculate_derived_stats()
    
    def get_option_value(self, option_key, index):
        """Get the actual value for an option at the given index."""
        option_config = CHARACTER_CONFIG["options"][option_key]
        values = option_config["values"]
        
        if isinstance(values[index], dict):
            return values[index]["value"] if "value" in values[index] else values[index]["name"]
        else:
            return values[index]
    
    def get_option_display_name(self, option_key, index):
        """Get the display name for an option at the given index."""
        option_config = CHARACTER_CONFIG["options"][option_key]
        values = option_config["values"]
    ...
    def update_character_data(self, option_key):
        ...
        index = self.selected_indices[option_key]
        self.character_data[option_key] = self.get_option_value(option_key, index)
        
        # Recalculate all derived stats whenever any option changes
        self.recalculate_derived_stats()
    
    def recalculate_derived_stats(self):
        """Recalculate all derived stats based on current selections."""
        # Reset to base values
        base_wealth = CHARACTER_CONFIG["defaults"]["wealth"]
        base_skill_points = CHARACTER_CONFIG["defaults"]["skill_points"]
        base_equipment_points = CHARACTER_CONFIG["defaults"]["equipment_points"]
        
        # Start with base values
        self.character_data["Wealth"] = base_wealth
        self.character_data["Skill Points"] = base_skill_points
        self.character_data["Equipment Points"] = base_equipment_points
        
        # Apply Origin bonuses (these override base values for equipment and wealth)
        if "origin" in self.selected_indices:
            origin_index = self.selected_indices["origin"]
            origin = CHARACTER_CONFIG["options"]["origin"]["values"][origin_index]
            self.character_data["Equipment Points"] = origin.get("equipment_points", base_equipment_points)
            self.character_data["Wealth"] = origin.get("wealth", base_wealth)
        
        # Apply Background bonuses (skill points override, wealth is additive)
        if "background" in self.selected_indices:
            background_index = self.selected_indices["background"]
            background = CHARACTER_CONFIG["options"]["background"]["values"][background_index]
            self.character_data["Skill Points"] = background.get("skill_points", base_skill_points)
            self.character_data["Wealth"] += background.get("wealth", 0)
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        console.print(
            statbox_x,
            statbox_y + 3,
            f"Wealth: {self.character_data['Wealth']}",
            fg=color.white,
            bg=(0, 0, 0)
        )
        console.print(
            statbox_x,
            statbox_y + 4,
            f"Skill Points: {self.character_data['Skill Points']}",
            fg=color.white,
            bg=(0, 0, 0)
        )
        console.print(
            statbox_x,
            statbox_y + 5,
            f"Equipment Points: {self.character_data['Equipment Points']}",
            fg=color.white,
            bg=(0, 0, 0)
        )
        ...
        for i, option in enumerate(self.menu_options):
            ...
            elif option in ["Symbol", "Color", "Origin", "Background"]:
            ...
            # Show color indicator for color option
            if option == "Color":
                console.print(
                    box_x + 3 + len(text) - 1,
                    menu_start_y + i,
...
class MainMenu(input_handlers.BaseEventHandler):
    ...
    def ev_keydown(
        ...
        if event.sym in (tcod.event.KeySym.Q, tcod.event.K_ESCAPE):
            ...
            return CharacterCustomizationScreen(self)
